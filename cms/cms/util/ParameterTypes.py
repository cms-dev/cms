...
""" A collection of parameter type descriptions that are supported by AWS.

These parameter types can be used to specify the accepted parameters
of a task type or a score type. These types can cover "basic" JSON values,
as task_type_parameters and score_parameters are represented by JSON objects.
"""
...
class ParameterType:
    ...
    def __init__(self, name, short_name, description):
        ...
        short_name (string): Short name without spaces, used for HTML
            element ids.
        description (string): Describes the usage and effect of this parameter.
        """
        self.name = name
