...
def get_compilation_command(language, source_filename, executable_filename):
    ...
    # programs).
    if language == "c":
        command = ["/usr/bin/gcc-4.5", "-DEVAL", "-static", "-O2", "-lm", "-o", executable_filename, source_filename]
    elif language == "cpp":
        command = ["/usr/bin/g++-4.5", "-DEVAL", "-static", "-O2", "-o", executable_filename, source_filename]
    elif language == "pas":
