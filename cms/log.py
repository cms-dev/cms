...
class CustomFormatter(logging.Formatter):
    ...
    def get_coordinates(self, record):
        ...
        non detailed log, it only contains service coordinates and
        operation, if set.
        ...
        if hasattr(record, "service_name") and \
            ...
            coordinates = "%s,%d" % (
                record.service_name.replace("Service", ""),
                record.service_shard)
        else:
            coordinates = "None"

        # Operation
        coordinates += ("/%s" % (record.operation)) \
            if hasattr(record, "operation") else ""
