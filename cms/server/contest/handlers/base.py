    ...
    def get_current_user(self):
        ...
        if self.contest.ip_autologin:
            ...
            if self.contest.block_hidden_participations:
                participations = participations.filter(Participation.hidden == False)
            ...
            # Having more than participation with the same IP, is a mistake and should not happen.
            # We disallow login for that IP completely, in order to make sure the problem is noticed.
            if len(participations) > 1:
                logger.error("Multiple participants have IP %s while auto-login feature is enabled." % (remote_ip))
                return None
