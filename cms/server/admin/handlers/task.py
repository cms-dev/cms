...
class AddTaskHandler(SimpleHandler("add_task.html", permission_all=True)):
    ...
    def post(self):
        fallback_page = self.make_unprefixed_absolute_href("tasks", "add")
...
class AddStatementHandler(BaseHandler):
    ...
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self, task_id):
        fallback_page = self.make_unprefixed_absolute_href("task", task_id, "statements", "add")
...
class AddAttachmentHandler(BaseHandler):
    ...
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self, task_id):
        fallback_page = self.make_unprefixed_absolute_href("task", task_id, "attachments", "add")
...
class AddDatasetHandler(BaseHandler):
    ...
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self, task_id):
        fallback_page = self.make_unprefixed_absolute_href("task", task_id, "add_dataset")
...
class TaskListHandler(SimpleHandler("tasks.html")):
    ...
    def post(self):
        ...
        if operation == self.REMOVE:
            asking_page = self.make_unprefixed_absolute_href("tasks", task_id, "remove")
            # Open asking for remove page
            self.redirect(asking_page)
        else:
            self.application.service.add_notification(
                make_datetime(), "Invalid operation %s" % operation, "")
            self.redirect(self.make_unprefixed_absolute_href("tasks"))
