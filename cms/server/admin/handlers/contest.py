...
class AddContestHandler(
    ...
    def post(self):
        fallback_page = self.make_unprefixed_absolute_href("contests", "add")
        ...
        if self.try_commit():
            ...
            self.redirect(self.make_unprefixed_absolute_href("contest", contest.id))
        else:
            self.redirect(fallback_page)
...
class ContestHandler(SimpleContestHandler("contest.html")):
    ...
    def post(self, contest_id):
        ...
        try:
            ...
            self.application.service.add_notification(
                make_datetime(), "Invalid field(s).", repr(error))
            self.redirect(self.make_unprefixed_absolute_href("contest", contest_id))
            return
...
class ContestListHandler(SimpleHandler("contests.html")):
    ...
    def post(self):
        ...
        if operation == self.REMOVE:
            asking_page = self.make_unprefixed_absolute_href("contests", contest_id, "remove")
