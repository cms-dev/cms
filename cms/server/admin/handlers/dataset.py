...
class CloneDatasetHandler(BaseHandler):
    ...
    def post(self, dataset_id_to_copy):
        fallback_page = self.abs_url("dataset", dataset_id_to_copy, "clone")
        ...
        if self.try_commit():
            self.redirect(self.abs_url("task", task_id))
        else:
            self.redirect(fallback_page)
...
class RenameDatasetHandler(BaseHandler):
    ...
    def post(self, dataset_id):
        fallback_page = self.abs_url("dataset", dataset_id, "rename")
        ...
        if self.try_commit():
            self.redirect(self.abs_url("task", task.id))
        else:
            self.redirect(fallback_page)
...
class AddManagerHandler(BaseHandler):
    ...
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self, dataset_id):
        fallback_page = self.abs_url("dataset", dataset_id, "managers", "add")
...
class AddTestcaseHandler(BaseHandler):
    ...
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self, dataset_id):
        fallback_page = self.abs_url("dataset", dataset_id, "testcases", "add")
...
class AddTestcasesHandler(BaseHandler):
    ...
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self, dataset_id):
        fallback_page = self.abs_url("dataset", dataset_id, "testcases", "add_multiple")
...
class DownloadTestcasesHandler(FileHandler):
    ...
    @require_permission(BaseHandler.AUTHENTICATED)
    def post(self, dataset_id):
        fallback_page = self.abs_url("dataset", dataset_id, "testcases", "download")
