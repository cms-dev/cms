...
class DownloadTestcasesHandler(FileHandler):
    """Download all testcases with compressed zip file.
    ...
    def post(self, dataset_id):
        ...
        # Get zip file name, input/output file names templates, or use default ones.
        zip_filename = self.get_argument("zip_filename", None)
        if not zip_filename:
        ...
        with zipfile.ZipFile(zip_path, "w") as zip_file:
            for testcase in dataset.testcases.itervalues():
                ...
                input_path = self.application.service.file_cacher.get_file(testcase.input).name
                output_path = self.application.service.file_cacher.get_file(testcase.output).name
                zip_file.write(input_path, input_template % testcase.codename)
                zip_file.write(output_path, output_template % testcase.codename)
            zip_file.close()
