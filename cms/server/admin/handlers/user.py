...
class UserHandler(BaseHandler):
    ...
    def post(self, user_id):
        fallback_page = self.make_unprefixed_absolute_href("user", user_id)
...
class UserListHandler(SimpleHandler("users.html")):
    ...
    def post(self):
        ...
        if operation == self.REMOVE:
            asking_page = self.make_unprefixed_absolute_href("users", user_id, "remove")
            # Open asking for remove page
            self.redirect(asking_page)
        else:
            self.application.service.add_notification(
                make_datetime(), "Invalid operation %s" % operation, "")
            self.redirect(self.make_unprefixed_absolute_href("contests"))
...
class TeamHandler(BaseHandler):
    ...
    def post(self, team_id):
        fallback_page = self.make_unprefixed_absolute_href("team", team_id)
...
class AddTeamHandler(SimpleHandler("add_team.html", permission_all=True)):
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self):
        fallback_page = self.make_unprefixed_absolute_href("teams", "add")
...
class AddUserHandler(SimpleHandler("add_user.html", permission_all=True)):
    @require_permission(BaseHandler.PERMISSION_ALL)
    def post(self):
        fallback_page = self.make_unprefixed_absolute_href("users", "add")
        ...
        if self.try_commit():
            ...
            self.application.service.proxy_service.reinitialize()
            self.redirect(self.make_unprefixed_absolute_href("user", user.id))
        else:
            self.redirect(fallback_page)
