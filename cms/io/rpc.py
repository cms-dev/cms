...
class RemoteServiceBase(object):
    ...
    def _read(self):
        ...
        If the returned string does not end with "\\r\\n" then it has
        been truncated because it was longer than MAX_MESSAGE_SIZE (or
        EOF has been reached). Successive reads will obtain the other
...
class RemoteServiceServer(RemoteServiceBase):
    ...
    def __init__(self, local_service, remote_address):
        ...
        self.pending_incoming_requests = dict()
        self.pending_incoming_requests_threads = dict()

    def finalize(self, reason=""):
        """See RemoteServiceBase.finalize."""
        super(RemoteServiceServer, self).finalize(reason)

        for thread in self.pending_incoming_requests_threads.itervalues():
            thread.kill(RPCError(reason), block=False)

        self.pending_incoming_requests.clear()
    ...
    def process_incoming_request(self, request):
        ...
        # Store the request.
        self.pending_incoming_requests[id_] = request
        self.pending_incoming_requests_threads[id_] = gevent.getcurrent()
