...
class ProxyService(TriggeredService):
    ...
    def operations_for_score(self, submission):
        ...
        return self.operations_for_submission(submission) + [
    ...
    def submission_submitted(self, submission_id):
        ...
        with SessionGen() as session:
            ...
            if submission is None:
                logger.error("[submission_submitted] Received submission "
                             "request for unexistent submission id %s.",
                             submission_id)
                raise KeyError("Submission not found.")

            if submission.participation.hidden:
                logger.info("[submission_submitted] Information of submission "
                            "%d not sent because the participation is hidden.",
                            submission_id)
                return
