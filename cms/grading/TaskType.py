...
def eval_output(file_cacher, job, use_checker,
    ...
    use_checker (bool): True to use a checker, False to use white diff.
    ...
    if use_checker:
        ...
        # Put reference solution, input, and user output in the sandbox.
        sandbox.create_file_from_storage("correct.txt", job.output)
        sandbox.create_file_from_storage("input.txt", job.input)
        if user_output_path is not None:
            shutil.copyfile(
        ...
        success, outcome, text = checker_step(
            sandbox, job.managers, "input.txt", "correct.txt", "output.txt")
