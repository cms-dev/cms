...
class TwoSteps2019(TaskType):
    ...
    The admins must provide a grader source file (for each language),
    called grader.%l, that get compiled with both two user sources,
    ...
    the two sources (manager and user provided).
    ...
    def _uses_checker(self):
        return self.output_eval == TwoSteps2019.OUTPUT_EVAL_CHECKER
    ...
    def compile(self, job, file_cacher):
        ...
        files_to_get[manager_filename] = job.managers[manager_filename].digest
        # Manager's header.
        if header_ext is not None:
            manager_filename = "grader%s" % header_ext
            if check_manager_present(job, manager_filename):
                source_filenames.append(manager_filename)
                files_to_get[manager_filename] = job.managers[manager_filename].digest
        ...
        for filename, file_ in job.files.items():
            ...
            if header_ext is not None:
                ...
                if not check_manager_present(job, header_filename):
                    return
                source_filenames.append(header_filename)
                files_to_get[header_filename] = job.managers[header_filename].digest

        # Get compilation command.
        executable_filename = "grader"
        commands = language.get_compilation_commands(source_filenames, executable_filename)
