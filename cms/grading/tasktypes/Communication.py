...
class Communication(TaskType):
    ...
    functions provided by the stub, will read communicate with the manager.
    Finally, the manager will decide outcome and text, and print them on
    stdout and stderr.
    ...
    first from and to the first user process, than from and to the second user
    process, and so on.
    ...
    """
    # Codename of the manager (the stand-alone, admin-provided program).
    MANAGER_CODENAME = "manager"
    ...
    def get_compilation_commands(self, submission_format):
        ...
        for language in LANGUAGES:
            ...
            for file_format in submission_format:
                source_filenames.append(file_format.replace(".%l", source_ext))
            # Compute executable name.
            executable_filename = "_".join(
                file_format.replace(".%l", "")
                for file_format in submission_format)
    ...
    def evaluate(self, job, file_cacher):
        ...
        if not check_manager_present(job, Communication.MANAGER_CODENAME):
            return
        manager_digest = job.managers[Communication.MANAGER_CODENAME].digest
        ...
        sandbox_mgr.create_file_from_storage(
            Communication.MANAGER_CODENAME, manager_digest, executable=True)
        sandbox_mgr.create_file_from_storage(
        ...
        manager_command = ["./%s" % Communication.MANAGER_CODENAME]
        for i in indices:
