...
\begin{multicols}{2}
  ...
  in Italian. This, of course, is going to change.
  ...
  \item \id{fake} (string): when set to ``\id{True}'' (at the moment
    this is case-sensitive) set the \id{hidden} flag in the user;
    defaults to false.
  ...
  \item \id{risultati} (``results'', string): a comma-separated list
    ...
    $\text{\id{n\_input}}-1$) that are available for live evaluation to
    users during the contest (the unpacked list is copied in the
    \id{public\_testcases} field);
  ...
  Here we describe all terms that have a specific meaning inside
  \CMS{}.
