...
\begin{multicols}{2}
  ...
  \item \id{token\_num} (integer): the maximum number of tokens
    that can be used by a contestant during the whole contest; setting
    it to zero means not enforcing this limit;

  \item \id{token\_min\_interval} (float): the minimum interval
    (in minutes) between using two tokens;

  \item \id{token\_gen\_time} (float): the time (in minutes) after
    which a new token is made available to the contestant; more
    precisely, each \id{token\_gen\_time} minutes a new token is
    available, until \id{token\_num} of them are created; setting
    this time to zero, of course, implies that all tokens are
    immediately available at the beginning of the contest;
  ...
  \item \id{token\_num} (integer), \id{token\_min\_interval} (float),
    \id{token\_gen\_time} (float): these parameters have the same
    meaning of those present in \id{Contest}, but are specific for
    this task.
  ...
  \item \id{user} (string): the contastant that made this submission;
