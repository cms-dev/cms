...
TEMPLATE = {
    LANG_C: "/**" + _raw_template_data + "*/\n",
    LANG_PASCAL: "(**" + _raw_template_data + "*)\n",
    LANG_PYTHON: "\"\"\"" + _raw_template_data + "\"\"\"\n",
    LANG_PHP: "<?php\n/**" + _raw_template_data + "*/\n?>",
}
...
def filter_top_scoring(results, unique):
    ...
    for key, value in usertask.iteritems():
        if unique:
            ...
            for v in value:
                results.append(v[2])  # the "old" row
...
def main():
    ...
    with SessionGen() as session:
        ...
        if args.unique or args.best:
            result = filter_top_scoring(results, args.unique)

        print("%s file(s) will be created." % len(result))
