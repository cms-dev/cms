    ...
    def run(self):
        for k, v in self.objs.iteritems():
            ...
            if v["_class"] == "Dataset":
                ...
                elif v["score_type"] not in {"GroupMin", "GroupMul",
                                             "GroupThreshold"} \
                        and not isinstance(v["score_type_parameters"], list):
                    logger.warning("A Dataset uses a custom score type, `%s', "
                                   "whose parameters are not in list format. "
                                   "It needs to be manually updated to keep "
                                   "working properly.", v["score_type"])
