...
def encrypt_binary(pt, key_hex):
    ...
    key_hex (string): a 16-bytes key in hex (a string of 32 hex chars).

    return (string): pt encrypted using the key, in a format URL-safe
...
def decrypt_binary(ct_b64, key_hex):
    ...
    ct_b64 (string): the ciphertext as produced by encrypt_binary.
    key_hex (string): the 16-bytes key in hex format used to encrypt.
